...
The *description in text* is the validator's message, passed through unaltered. This library does not write error prose and cannot judge it: zod 4.6.4's default for an empty required string is `Too small: expected string to have >=1 characters`, which is text and does describe the error, and is also not a sentence anybody should show a person. The contract carries `FormIssue.message` and the vendor owns it.
