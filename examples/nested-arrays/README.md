...
`row.path` は `form:shipments[0]`。内側のリストは「自分が内側である」ことを
知りません。**入れ子専用の API はありません** — `useRows` は具体パスを取るので、
内側は外側の行が既に配った住所を使うだけです。住所は所属フォーム名まで含んだ
1本の文字列なので、横に添えて渡すものは何もありません。
